...
GGR_Store_results<-function(GAME, QUESTION, ANSWER, CORRECT){
  ...
  if("Your_GeoGuessR_results.csv" %not in% list.files()){
    results<-data.frame(Game=NA, Date=NA, Question=NA, Answer=NA, Correct=NA)
  } else {
    results<-read.csv("Your_GeoGuessR_results.csv")
  }
  ...
  ### Save the file
  write.csv(results[is.na(results$Game)==F,], "Your_GeoGuessR_results.csv", row.names=F)
